...
const MARKED_GRADES = new Set(['great', 'mistake', 'blunder']);
...
export default function EvalChart({
  ...
  const geometry = useMemo(() => {
    ...
    const x = (index) => PLOT_INSET.left + (index / span) * usableWidth;
    const y = (percent) =>
    ...
    const firstY = y(points[0].redPercent);
    const lastY = y(points[points.length - 1].redPercent);
  ...
  const marks = useMemo(
    () =>
      geometry
        ? moves
            .filter((move) => !move.pending && MARKED_GRADES.has(move.grade.key))
            .map((move) => ({
              key: move.index,
              grade: move.grade,
