...
export default function PGNImportModal({ onClose, onLoad, visible }) {
  ...
  const pasteFromClipboard = async () => {
    try {
      ...
      setError(clipboardError.message || 'The clipboard could not be read.');
    }
  };
  ...
  const load = () => {
    ...
    try {
      ...
      setError(loadError.message || 'This PGN could not be loaded.');
    }
  };
...
const styles = StyleSheet.create({
  ...
  backdrop: { ...StyleSheet.absoluteFillObject, backgroundColor: overlay },
  card: {
    width: '100%',
    maxWidth: 620,
    maxHeight: '94%',
