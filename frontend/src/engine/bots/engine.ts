import { allValidMoves, enginePosition, type AnalysisGame, type PositionLike } from '../analysisGame';
import { analyzeExclusive, type AnalyzeFn } from '../rpsfish/client';
...
import {
  ...
  mannersScoresFor,
  type BotChoice,
  type BotChoiceScores,
  type BotProfile,
  type BotSearchLimits,
} from './profiles';
...
// these, and `bots/arena.ts` drives two against each other. Everything that
// varies between difficulties comes from a profile in `bots/profiles.ts`.
//
 ...
 */
export const createSeededRandom = (seed: number | bigint | string): RandomSource => {
...
/** One move the search offered, with where it came in the ranking. */
interface Candidate extends Move {
  score: number;
  rank: number;
}
...
const sampleCandidate = (
  ...
  if (!best) throw new Error('A move cannot be sampled from an empty candidate set.');
  if (candidates.length === 1 || !(temperatureCp > 0)) return best;
  const weights = candidates.map((candidate) =>
    Math.exp((candidate.score - best.score) / temperatureCp),
  );
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    if (ticket <= 0) return candidates[index] ?? best;
  }
  return candidates[candidates.length - 1] ?? best;
};

const candidateLineCount = (choice: BotChoice) =>
...
const rankedCandidates = (
  ...
  choice: BotChoiceScores,
): Candidate[] => {
  const lines: Candidate[] = (analysis?.lines ?? [])
    .slice(0, Math.max(1, Math.min(choice.candidateLines, BOT_TUNING.maxCandidateLines)))
    .map((line, index) => ({
      from: line.from,
      to: line.to,
      score: line.score,
  ...
  const withinGuardrail = lines.filter((line) => best.score - line.score <= choice.maxLossCp);
  return withinGuardrail.length > 0 ? withinGuardrail : [best];
};

const randomLegalMove = (game: AnalysisGame, random: RandomSource): Move | null => {
  const moves = allValidMoves(game);
  if (moves.length === 0) return null;
  return moves[Math.min(moves.length - 1, Math.floor(random() * moves.length))] ?? null;
};

/** What a bot decided, and everything the caller needs to show why. */
export interface BotDecision {
  analysis: Analysis | null;
  from: Position;
  to: Position;
  /** Where the played move came in the search's ranking, if it came from one. */
  rank: number | null;
  reason: 'best' | 'sampled' | 'random' | 'fallback';
  score: number | null;
  elapsedMs: number;
  /**
   * Whether the position is past this profile's resignation threshold. Reported
   * rather than acted on: the caller decides when a bot game actually ends.
   */
  resigns: boolean;
}
...
export interface BotSearchOptions {
  history?: PositionLike[];
  signal?: AbortSignal;
  limits?: BotSearchLimits;
}

export interface Bot {
  acceptsDraw: (game: AnalysisGame, scoreForBotCp: number | null | undefined) => boolean;
  blurb: string;
  chooseMove: (game: AnalysisGame, options?: ChooseMoveOptions) => Promise<BotDecision | null>;
  id: string;
  name: string;
  profile: BotProfile;
  rating: number;
  search: (game: PositionLike, options?: BotSearchOptions) => Promise<Analysis>;
}

export interface CreateBotOptions {
  analyze?: AnalyzeFn;
  /**
   * Evaluate the position even on a move the profile chose at random, so a
   * watched battle's chart and grades stay complete.
   */
  analyzeRandomMoves?: boolean;
  now?: () => number;
  random?: RandomSource;
...
export const createBot = (
  profileOrId: string | BotProfile | undefined,
   ...
   * legal move; otherwise the decision, including the analysis behind it. The
  ...
  const chooseMove = async (
    game: AnalysisGame,
    { history = [], signal }: ChooseMoveOptions = {},
    ...
    let chosen: Move | Candidate | null = null;
    let reason: BotDecision['reason'] = 'random';
    ...
    if (rolledRandom) {
      ...
      if (fallback) {
        ...
        const sampled = sampleCandidate(candidates, choice.temperatureCp, random);
        chosen = sampled;
        reason = sampled.rank === 1 ? 'best' : 'sampled';
      }
    }
    ...
    const thinkMs =
      tempo.minThinkMs + random() * Math.max(0, tempo.maxThinkMs - tempo.minThinkMs);
    await sleep(Math.max(0, thinkMs - (now() - startedAt)));
    if (signal?.aborted) throw abortError();

    const score = 'score' in chosen ? chosen.score : null;
    return {
      analysis,
      from: chosen.from,
      rank: 'rank' in chosen ? chosen.rank : null,
      reason,
      score,
      to: chosen.to,
      ...
      resigns:
        ...
        typeof score === 'number' &&
        Math.abs(score) < MATE_SCORE_FLOOR &&
        score <= manners.resignBelowCp,
    };
  };
   ...
   */
  const acceptsDraw = (game: AnalysisGame, scoreForBotCp: number | null | undefined) => {
