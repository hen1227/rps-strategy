...
export type AnalysisPreset = keyof typeof ANALYSIS_PRESET_LABELS;

export const DEFAULT_ANALYSIS_PRESET: AnalysisPreset = 'standard';
 ...
 */
export const WATCHED_GAME_PRESET: AnalysisPreset = 'deep';
 ...
 */
export type AnalysisStatus = 'idle' | 'running' | 'waiting' | 'done' | 'error';
...
export interface CreateGameAnalysisOptions {
  ...
  /**
   * A key of `presets`. Deliberately wider than `AnalysisPreset`: that union is
   * what the UI offers, while a test or a measurement script supplies a presets
   * map of its own and names a budget that no screen has a button for.
   */
  preset?: string;
...
export const createGameAnalysis = ({
  ...
  const limits = presets[preset] ?? presets[DEFAULT_ANALYSIS_PRESET] ?? REVIEW_PRESETS.standard;
  ...
  const request = () => {
    ...
    review(
      ...
      .catch((failure: unknown) => {
        ...
        if (stopped || named?.name === 'AbortError') return;
