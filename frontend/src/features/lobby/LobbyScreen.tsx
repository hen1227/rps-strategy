...
import BotSeriesPanel from '@/features/bots/BotSeriesPanel';
import EngineBotRow from '@/features/bots/EngineBotRow';
...
const formatSearchTime = (milliseconds: number) => `${Math.floor(milliseconds / 1000)}s`;

const playerName = (profile: PlayerProfile | null | undefined, fallback: string) => {
...
function BotLevelPicker({ label, onSelect, selectedProfileId }: BotLevelPickerProps) {
  return (
    <View>
...
export default function LobbyScreen() {
  ...
  const [challengePosition, setChallengePosition] = useState<StartingPosition | null>(null);
  const [challengePositionOpen, setChallengePositionOpen] = useState(false);
  const [pgnModalOpen, setPgnModalOpen] = useState(false);
  const [engineBotModeId, setEngineBotModeId] = useState<ModeID | null>(null);
  ...
  const submitChallenge = (mode: ModeDefinition) => {
    ...
    <View style={styles.topBar}>
      ...
      <View style={styles.topBarActions}>
        ...
        <GhostButton
          ...
          compact
          label="OPENINGS"
          onPress={() => router.push(links.openings())}
        />
        <GhostButton
            ...
            <View key={mode.id} style={styles.modeActionButton}>
              <PrimaryButton
                ...
                onPress={() =>
                  router.push(
                    links.botBattle({
                      blue: battleBlueProfile.id,
                      mode: mode.id,
                      red: battleRedProfile.id,
                    }),
                  )
                }
              />
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'right', 'bottom', 'left']}>
      ...
      <PGNImportModal
        ...
        onLoad={(pgn) => {
          ...
          // A pasted record has no game id, so it is handed over rather than
          // addressed. See `store/reviewHandoff.ts`.
