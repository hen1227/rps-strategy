...
export interface TerritoryMeterProps {
  grid: Grid;
}

export default function TerritoryMeter({ grid }: TerritoryMeterProps) {
  ...
  const majorityOwner: SideColor | null =
    counts.red >= majorityThreshold
      ? 'Red'
      : counts.blue >= majorityThreshold
        ? 'Blue'
        : null;
  const majorityColor = majorityOwner ? players[majorityOwner].territory : undefined;
