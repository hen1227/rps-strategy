...
} from '../engine/openingBook.mjs';
import { Badge, Banner, GhostButton, Panel, PrimaryButton } from '../components/ui';
import {
...
const mergeName = (names, published) => [
  ...(names ?? []).filter(
...
function NameForm({ busy, inheritedFrom, line, onSubmit, publishedName }) {
...
function AdminStudio({
  ...
  const adminToken = admin.token || null;
  ...
  const importJSON = async () => {
    ...
    try {
      ...
      await importOpeningBook(adminToken, modeId, parsed);
      setJsonDraft('');
      await onBookImported();
      onNotice(`Imported ${parsed.positionCount.toLocaleString()} analyzed positions.`);
    } catch (requestError) {
      setError(requestError instanceof SyntaxError ? 'That is not valid JSON.' : requestError.message);
    } finally {
      setBusy(null);
    }
  };
...
export default function OpeningBookScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (requestError.status === 404) setBookData(null);
  ...
  const submitSuggestion = async (name) => {
    setSuggesting(true);
    setError(null);
    try {
  ...
  const publishLocally = (published) => {
    setBookData((current) => ({
      ...current,
      names: mergeName(current?.names, published),
    }));
  };
      ...
      >
        <View style={styles.screen}>
          <View style={styles.header}>
            <GhostButton compact label="← LOBBY" onPress={() => navigation.navigate('Lobby')} />
