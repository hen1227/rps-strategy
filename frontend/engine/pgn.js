...
const PIECE_LETTERS = { Rock: 'R', Paper: 'P', Scissors: 'S' };
const LETTER_PIECES = { R: 'Rock', P: 'Paper', S: 'Scissors' };
...
const DEFEATS = { Rock: 'Scissors', Scissors: 'Paper', Paper: 'Rock' };
...
export const formatSquare = ({ x, y }) => {
  if (x < 0 || x >= BOARD_SIZE || y < 0 || y >= BOARD_SIZE) return '??';
  return `${FILES[x]}${y + 1}`;
};

export const parseSquare = (text) => {
...
export const formatMove = ({ piece, from, to, captured }, endsGame = false) => {
...
export const parseMoveToken = (token) => {
  ...
  let captured = 'Empty';
  if (match[3] === 'x') captured = match[4] ? LETTER_PIECES[match[4]] : DEFEATS[piece];
  return {
    piece,
...
const TURN_CODES = { r: 'Red', b: 'Blue', '-': 'Neutral' };

const decodeRows = (field, place) => {
 ...
 */
export const decodePosition = (text) => {
  ...
  if (fields.length === 0) throw new PGNError('empty position');

  decodeRows(fields[0], (x, y, symbol) => {
    const piece = SYMBOL_PIECES[symbol];
    if (!piece) throw new PGNError(`unsupported piece "${symbol}"`);
    grid[y][x].occupant = piece.occupant;
    grid[y][x].occupantOwner = piece.owner;
    grid[y][x].ownerColor = piece.owner;
  });

  let currentTurn = 'Neutral';
  if (fields.length > 1) {
    currentTurn = TURN_CODES[fields[1]];
    if (!currentTurn) throw new PGNError(`"${fields[1]}" is not a color`);
  }
  if (fields.length > 2) {
    for (const row of grid) for (const tile of row) tile.ownerColor = 'Neutral';
    decodeRows(fields[2], (x, y, symbol) => {
...
const encodeRows = (grid, symbolFor) =>
  grid
    .map((row) => {
      let text = '';
      let gap = 0;
...
const pieceSymbol = (tile) => {
  const letter = PIECE_LETTERS[tile.occupant];
  if (!letter) return '';
  return tile.occupantOwner === 'Red' ? letter.toLowerCase() : letter;
};

const TURN_SYMBOLS = { Red: 'r', Blue: 'b', Neutral: '-' };

export const encodePosition = (grid, currentTurn) =>
  [
    encodeRows(grid, pieceSymbol),
    TURN_SYMBOLS[currentTurn] ?? '-',
    encodeRows(grid, (tile) => (tile.ownerColor === 'Neutral' ? '' : TURN_SYMBOLS[tile.ownerColor])),
  ].join(' ');

/** The row form `createAnalysisGame` builds a board from. */
export const startingRowsFrom = (grid) =>
  grid.map((row) => row.map((tile) => pieceSymbol(tile) || '.').join(''));

// --- reading ---------------------------------------------------------------

const unescapeTagValue = (value) => value.replace(/\\(.)/g, (_, character) => character);
...
const tokenizeMovetext = (movetext) => {
  const tokens = [];
  let index = 0;
...
const parseComment = (token) => {
  const annotations = { elapsedMs: 0, redRemainingMs: null, blueRemainingMs: null };
  ...
  while (match) {
    ...
    switch (match[1]) {
      ...
      case 'clk':
        ...
        break;
      case 'act':
        annotations.action = { kind: parts[0], player: parts[1], bonusMs: Number(parts[2] ?? 0) };
        break;
      case 'end':
...
export const parsePGN = (text) => {
  ...
  for (const token of tokenizeMovetext(movetext)) {
    if (token.startsWith('{')) {
      ...
      if (annotations.end) {
        flush();
        events.push({ kind: 'end', ...annotations.end, ...annotations });
      } else if (annotations.action) {
        flush();
        events.push({ kind: 'action', ...annotations.action, ...annotations });
      } else if (pendingMove) {
        Object.assign(pendingMove, annotations);
        flush();
      }
      continue;
    }
    ...
    flush();
    pendingMove = { kind: 'move', player: color, ...parseMoveToken(token) };
  }
  flush();

  const declared = lookup.get('Result');
  if (declared && result === RESULT_UNFINISHED) result = declared;
  return {
    tags,
    tag: (name) => lookup.get(name) ?? '',
    events,
    result,
  };
};
...
const formatSecondsValue = (milliseconds) => {
...
const ADJUDICATED_BY_A_MOVE = new Set([
  'game_rule',
  'annihilation',
  'territory',
  'infiltration',
 ...
 * the archive.
 */
export const encodePGN = ({ tags = [], moves = [], endReason, winner, result }) => {
  const header = tags
...
export const resultFor = (status, winner) => {
...
export const winnerFromResult = (result) => {
