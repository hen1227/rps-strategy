...
//      `scripts/reviewCalibration.mjs` measures it.
...
import { allValidMoves, applyAnalysisMove, createAnalysisGameFrom } from './analysisGame';
import { decodePosition, parsePGN, winnerFromResult } from './pgn';
 ...
 */
export const WIN_PROBABILITY_SCALE = Object.freeze({
 ...
 */
export const winPercent = (score, modeId) => {
  ...
  const scale = WIN_PROBABILITY_SCALE[modeId] ?? DEFAULT_SCALE;
  const clamped = Math.max(-SCORE_CLAMP, Math.min(SCORE_CLAMP, score));
  return 100 / (1 + Math.exp(-scale * clamped));
};
 ...
 */
export const decisiveScore = (modeId) =>
  Math.log(19) / (WIN_PROBABILITY_SCALE[modeId] ?? DEFAULT_SCALE);
 ...
 */
export const moveAccuracy = (winPercentBefore, winPercentAfter) => {
...
const LOSS_GRADES = MOVE_GRADES.filter((grade) => !['best', 'great'].includes(grade.key));
const GOOD_MOVE_MAX_LOSS = MOVE_GRADES.find((grade) => grade.key === 'good').maxLoss;
 ...
 */
export const isOnlyGoodMove = (bestScore, secondBestScore, modeId) => {
 ...
 */
export const gradeMove = ({ lossPercent, isOnlyGoodMove: onlyGood = false, isTopMove }) => {
  if (isTopMove) return MOVE_GRADES[onlyGood ? 1 : 0];
  return LOSS_GRADES.find((grade) => lossPercent <= grade.maxLoss) ?? LOSS_GRADES[LOSS_GRADES.length - 1];
};
 ...
 */
export const combineAccuracies = (accuracies, weights) => {
  if (accuracies.length === 0) return null;
  const totalWeight = weights.reduce((total, weight) => total + weight, 0);
  const weightedMean =
    totalWeight > 0
      ? accuracies.reduce((total, value, index) => total + value * weights[index], 0) / totalWeight
...
const modeFor = (modes, modeId, variant) => {
 ...
 */
export const reviewSourceFromPGN = (pgnText, modes) => {
...
export const branchesFrom = (game) => allValidMoves(game);
 ...
 * how well it played.
 */
export const summarizeReview = ({ source, entries, bookPlies = 0 }) => {
  const modeId = source.mode.id;
  const bound = decisiveScore(modeId);
  const clamp = (score) => Math.max(-bound, Math.min(bound, score ?? 0));
  const graded = source.moves.map((move, index) => {
  ...
  for (const color of ['Red', 'Blue']) {
    ...
    if (own.length === 0 || own.some((move) => move.pending)) {
      accuracy[color] = null;
      continue;
    }
    accuracy[color] = {
      ...
      averageLossPercent:
        own.reduce((total, move) => total + move.lossPercent, 0) / own.length,
      averageLossCentipawns:
        own.reduce((total, move) => total + move.lossCentipawns, 0) / own.length,
      grades: MOVE_GRADES.reduce((counts, grade) => {
