...
//             only by `botArena.js --adjudicate`, which is off by default.
//
// ---------------------------------------------------------------------------
...
// per mode at depths 2, 4, 6 and 11 (`node scripts/botArena.mjs --spread`,
...
const FALLBACK_SCALE = MODE_SCORE_SCALE.V1;

export const modeScoreScale = (modeId) => MODE_SCORE_SCALE[modeId] ?? FALLBACK_SCALE;

/** Convert a profile's mode-relative choice knobs into this mode's scores. */
export const choiceScoresFor = (choice, modeId) => {
  const { choice: unit } = modeScoreScale(modeId);
  return {
    candidateLines: choice.candidateLines,
    maxLossCp:
      choice.maxLossUnits === Infinity ? Infinity : choice.maxLossUnits * unit,
...
export const mannersScoresFor = (manners, modeId) => {
...
export const botProfile = (profileId) =>
