...
import { reviewSourceFromPGN, summarizeReview } from './gameReview.js';

const MODES = [
  { id: 'V5', name: 'Total War', shortCode: 'V5', playable: true, features: ['territory'] },
];
...
/** Entries good enough to grade against: every move loses nothing. */
const perfectEntries = (count) =>
  Array.from({ length: count }, (unused, index) => ({
    index,
    baselineScore: 0,
    playedScore: 0,
    analysis: { redScore: 0, depth: 6, lines: [] },
  }));
...
describe('book plies', () => {
  ...
  it('marks the dealt moves and leaves the chosen ones alone', () => {
    ...
    const report = summarizeReview({
      source,
      entries: perfectEntries(source.moves.length),
  ...
  it('keeps dealt moves out of the accuracy each engine is credited with', () => {
    const source = reviewSourceFromPGN(SERIES_PGN, MODES);
    const entries = perfectEntries(source.moves.length);
    ...
    entries[0] = { ...entries[0], baselineScore: 900, playedScore: -900 };
    entries[1] = { ...entries[1], baselineScore: 900, playedScore: -900 };

    const withBook = summarizeReview({ source, entries, bookPlies: 4 });
    const withoutBook = summarizeReview({ source, entries, bookPlies: 0 });

    assert.equal(withBook.accuracy.Red.moveCount, 1, 'three Red moves, two of them book');
    assert.equal(withoutBook.accuracy.Red.moveCount, 3);
    assert.ok(
      withBook.accuracy.Red.accuracy > withoutBook.accuracy.Red.accuracy,
      'a blunder nobody made must not be counted against the engine',
    );
    assert.equal(withBook.accuracy.Red.averageLossPercent, 0);
  });
  ...
  it('still evaluates the dealt moves, so the chart has no hole in it', () => {
    ...
    assert.equal(report.moves[0].pending, false);
    assert.equal(typeof report.moves[0].playedScore, 'number');
  });
});
